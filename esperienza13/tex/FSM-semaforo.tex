...
\maketitle
\begin{abstract}
L'obiettivo di questa esperienza è la realizzazione di semplici macchine a stati finiti basate su FF D-Latch. In particolare si vuole realizzare un semaforo: in una prima parte alternerà gli stati verde, verde/giallo, rosso; in una seconda parte verrà aggiunta la possibilità di disabilitarlo tramite un segnale di enable portandolo ad alternare gli stati giallo/spento.

Si è quindi realizzato la stessa FSM scrivendo un programma per Arduino.
\end{abstract}
\input{strum.tex}
\input{no_enable.tex}
\input{p3.tex}
