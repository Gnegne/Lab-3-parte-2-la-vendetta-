...
\newcommand{\fig}[1]{\figurename{ \ref{tab:#1}}} %inserting reference to figures
\newcommand{\tab}[1]{\tablename{ \ref{fig:#1}}} % inserting reference to tables
\newcommand{\eqn}[1]{equazione \eqref{eq:#1}} % inserting reference to equation
\newcommand{\eqn}[1]{sezione \ref{sez:#1}} % inserting reference to section
...
\newcommand{\titleheads}[3][BF]{% comando comodo per ridurre il clutter iniziale
	\author{Gruppo #1 \\ Thomas Giannoni, Valerio Lomanto, Roberto Ribatti}
	\title{#2: #3}
	\lhead{#2}
	\chead{#3}
	\rhead{Gruppo #1}
...
\pagestyle{fancy}
\titleheads{Esercitazione N.-7}{Diametro di Rotoballe}
